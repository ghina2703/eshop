...
Misalnya, saya memberi nama field seperti `nameField`, `descriptionField`, dan `quantityField` yang jelas menunjukkan elemen apa yang dimaksud. Saya juga sangat memperhatikan **keterbacaan kode**. 
Prinsip **Single Responsibility Principle (SRP)** juga saya terapkan di sini. Contohnya method _testing_ seperti `createProduct_isSuccessful` hanya berfokus pada pengujian fitur pembuatan produk.
...
Namun, tentu ada beberapa bagian yang bisa diperbaiki. Misalnya, dalam pengaturan **baseUrl**, baiknya kalau URL tidak di-_hardcode_ di dalam kode, tapi menggunakan properti dari file konfigurasi Spring (`application.properties` atau `application.yml`) yang mungkin akan lebih mempermudah pengelolaan URL di berbagai environment.
...
Saya juga menggunakan **WebDriverWait** supaya bisa menunggu elemen muncul di halaman yang belum semuanya dimuat.
